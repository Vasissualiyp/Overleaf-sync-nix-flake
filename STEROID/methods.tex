...
% ---------------------------------------------------------------
% Tracked changes markup (accept/reject later with pyMergeChanges.py)
\usepackage{changes}
\definechangesauthor[name={Vasilii}]{VP}
% ---------------------------------------------------------------
...
% ---------------------------------------------------------------
\begin{abstract}
	\comment[id=VP]{abstract reviewed; claims match the verified benchmark + geometric capabilities}
...
% VERIFIED Buckingham1914: "the number i, of separate independent arguments of Phi, is
%   the maximum number of independent dimensionless products... i = n - k" — p.4.
...
% VERIFIED Bakarji2022: "three data-driven techniques that use the Buckingham Pi theorem
%   as a constraint"; BuckiNet "projects the input parameter space to a lower dimension"
%   — Abstract p.1. VERIFIED SalazarMeza2023: Buckingham Pi applied to ultrasonic
%   micro-injection molding — Abstract. VERIFIED Opdahl2021: "deriving non-dimensional Pi
%   variables using Buckingham's Pi Theorem" for IsoTruss buckling — p.2.
...
% VERIFIED Gorard2025: "there is no guarantee that the resulting combinations of
%   parameters will be physically valid, or even mathematically consistent" —
...
% NOTE BuckinghamPy2021: SoftwareX software paper (ScienceDirect, gold OA but blocked
%   from this machine) — no PDF in Zotero. Claim is that DA software exists for the
%   forward direction; supported by the software's existence (BuckinghamPy package).
%   Fetch PDF before final submission.
...
regions via Karush-Kuhn-Tucker (KKT) conditions.
% VERIFIED Bemporad2002: "partitioning the given parameter set X into N convex
%   polyhedral regions"; solution "linear on CR"; KKT via Basic Sensitivity Theorem
%   — Algorithm 1, p.9.
The resulting lookup table
supports $O(\log R)$ query cost ($R$ = number of regions) via binary search
trees \cite{Tondel2003, Fuchs2010}.
% VERIFIED Fuchs2010: "a method to create a binary search tree for point location in
%   such polytopic sets, in order to provide a fast lookup of the control law" — Abstract.
% NOTE Tondel2003: original BST-for-PWA paper (Automatica); PDF not in Zotero, but the
%   fact is supported by Fuchs2010 (PDF verified).
The Delaunay Triangulation approach (a big part of
STEROID method) was also explored for explicit MPC by \cite{Scibilia2009}.
% NOTE Scibilia2009: no PDF obtainable (ECC paywalled); the Delaunay-for-MPC idea is
%   corroborated by the broader explicit-MPC literature, but this specific paper could
%   not be verified — consider replacing with a PDF-backed source.
...
%solver \cite{HiGHS2018} provides a state-of-the-art LP simplex and interior-point
...
Given a query $\mathbf{d} \in \mathbb{R}^m$, an algorithm that solves the
inverse problem should find $\mathbf{p} \in \mathbb{R}^n$ satisfying
\eqref{eq:constraint} and \eqref{eq:buckingham}, or report infeasibility, i.e.
return $\emptyset$ if $\mathbf{d} \notin \mathcal{D}^*$.

In practice, queries arrive as a batch $G = \{\mathbf{d}_1, \ldots, \mathbf{d}_Q\}$
(e.g., a regular $N^m$-point Cartesian grid in $[-1,1]^m$ in one example), and
the problem parameters $A, \Lambda, \mathbf{c}$ are fixed across all queries.
...
\label{sec:lp}
\comment[id=VP]{section checked; complexity numbers need a citation}

The most direct approach: solve \eqref{eq:reduced_lp} as an LP feasibility problem
with HiGHS \cite{HiGHS2018}.  Simplex is exponential in the worst case but
...
% VERIFIED HiGHS2018: HiGHS implements the dual revised simplex method for LPs —
%   §2.1 p.3. NOTE: the worst-case exponential simplex behavior and the interior-point
%   O((n-m)^3.5 log(1/eps)) exponent are standard LP results NOT stated in HiGHS2018;
%   the O((n-m)K) typical pivot count and the 3.5 exponent should be re-cited to a
%   complexity reference (e.g. interior-point analysis) before submission.

\subsection{Chebyshev Center LP}
\label{sec:chebyshev}
\comment[id=VP]{section checked; Boyd2004 verified}
...
% VERIFIED Boyd2004: "the largest Euclidean ball that lies in a polyhedron...
%   (The center of the optimal ball is called the Chebyshev center)" — §8.5.1.
% NOTE: the Zotero attachment for Boyd2004 is a Russian convex-optimization textbook
%   (user can read Russian); the claim was verified against the actual book text.
Useful when downstream applications require $\mathbf{p}$ to be far from
constraint boundaries.

\subsection{Dykstra's Alternating Projection}
\label{sec:dykstra}
\comment[id=VP]{section checked; Dykstra1983 verified}
...
% VERIFIED Dykstra1983: "simple iterative technique" minimizing ||g - x||^2 over
%   x in K1 ∩ ... ∩ Kr via cyclic projections with incremental correction terms —
%   p.2 / p.838.
...
\comment[id=VP]{section checked; ADMM splitting + Kaczmarz rate verified}
...
% VERIFIED BoydADMM2011: "decomposition-coordination procedure, in which the solutions
%   to small local subproblems are coordinated to find a solution to a large global
%   problem" — Introduction, p.7.
ADMM converges in $O(1/\varepsilon)$
iterations for the feasibility residual.
% NOTE: the O(1/eps) rate is not stated in BoydADMM2011 (which proves convergence
%   but gives no explicit rate for feasibility) — either cite a convergence analysis
%   paper with a rate, or soften to "ADMM converges for convex feasibility".
...
number of $\Lambda N$.
% VERIFIED Strohmer2009: "the error ||x_k - x||^2 reduces at each step in average...
%   by at least the factor of (1 - kappa(A)^{-2})" — p.3; "expected exponential rate"
%   — Abstract p.1.
...
\label{sec:steroid}
\comment[id=VP]{section checked; PPL/McMullen/CGAL citations annotated}
...
Library \cite{PPL2008} (see also the reverse-search pivoting algorithm of
\cite{Avis1992}).
% VERIFIED PPL2008: "vertex/facet enumeration... is particularly relevant for
%   implementations based on the Double Description method such as the Parma Polyhedra
%   Library" — p.23. VERIFIED Avis1992: pivoting (reverse-search) algorithm for vertex
%   enumeration — abstract.
By McMullen's Upper Bound Theorem \cite{McMullen1970},
...
% NOTE McMullen1970: original UBT paper (Mathematika); no PDF in Zotero (Wiley). The
%   upper bound theorem is a standard result in polytope theory; the exact vertex bound
%   is the classical McMullen statement. Consider adding a PDF-backed polytope-theory
%   reference (e.g. a survey) before submission.
...
\comment[id=VP]{checked; CGAL/McMullen annotated}
...
% NOTE cgal: software manual (online), no PDF needed for software usage claim.
% NOTE: worst-case Delaunay simplex count follows from the upper bound theorem
%   (McMullen1970).
...
% VERIFIED volesti: RDHR hit-and-run walk; mixing time O*(d^3) for H-polytopes —
%   Table 1, p.4; burn-in parameter "burns the first 1000 points RDHR generates" — p.6.
...
In explicit MPC \cite{Tondel2003, Fuchs2010}, a similar map is produced via KKT critical
regions, where each region corresponds to exactly one active set by construction.
% NOTE Tondel2003: no PDF; the phase-diagram-from-critical-regions claim is standard
%   explicit-MPC theory; Fuchs2010 (PDF verified) covers point location in the polytopic
%   partition.
...
% NOTE LaiSchumaker2007: CUP book, no PDF in Zotero; spline-on-triangulation theory is
%   standard (book is the canonical reference). NOTE Nesterov1998: MOR paywalled; the
%   analytic-center concept is standard interior-point theory.
...
future work.
% NOTE Ciarlet2002: the Zotero attachment is a different paper (arXiv:1709.09122
%   "Aggregated unfitted FEM"), NOT Ciarlet's book. The h^{k+1} interpolation-error
%   claim is standard FEM theory; needs the correct book PDF or a replacement citation.
...
% VERIFIED SalazarMeza2023: Buckingham Pi power-law fits for a manufacturing process
%   (energy Q and Young's modulus E in UMIM) — eqs (26),(28) p.15-16.
% VERIFIED RomeroObon2025: dimensionless QbD + Buckingham Pi as pharmaceutical
%   development accelerator — Abstract p.1, Conclusions §7 p.10.
...
% VERIFIED cvxpylayers: "an approach to differentiating through disciplined convex
%   programs... end-to-end analytical differentiation through the entire convex
%   program" — p.1.
...
code (Z-order curve) \cite{MortonCode1966, Berger2023}
of its bounding grid cell.  Per-query: one hash lookup
$O(1)$ + $O(m)$ barycentric containment check.
% NOTE MortonCode1966: original IBM tech report (no PDF). Z-order/Morton curves are a
%   general fact; Berger2023 (arXiv, PDF verified) documents Z-order curves.
% VERIFIED Berger2023: Z-order curves / Morton codes used for spatial indexing —
%   Abstract.
...
\begin{table}[h]
	...
	\end{tabular}
\end{table}
\comment[id=VP]{GPU FLOPs table: theoretical estimates; verify with hardware benchmarks.}
...
against HiGHS \cite{HiGHS2018}.
% NOTE: the previously-undefined \cite{HiGHS2022} key was replaced with \cite{HiGHS2018}
%   (the HiGHS solver paper, PDF verified).
...
% VERIFIED SalazarMeza2023: Pi groups are exactly (14)-(16): Pi0=Q/(Ut·P),
%   Pi1=A^4·Sf·TM·λ/(Ut·P^2), Pi2=A^3·E/(Ut·P) — p.7. The fitted exponents match
%   eq (28) (E) p.16 and eq (26) (Q) p.15.
\begin{align}
...
% VERIFIED SalazarMeza2023: the feasibility threshold "Pi1 greater than 10^-9"
%   is stated in the source (specimens must have Pi1 > 10^-9 to be complete) —
%   §4.1, p.14. NOTE: the box bounds are NOT all directly stated in the source:
%   the tested amplitude is 80-100% of the max 56.2 µm (Table 3); action time was
%   tested up to 6 s; clamping/injection force up to 6.5 kN. The values 22.5 µm,
%   U_t<=5 s and IF<=4000 N are the authors' chosen operating window, not source
%   numbers — confirm or re-derive before submission.
...
\comment[id=VP]{If time permits: implement the UMIM STEROID run numerically and show the
...
system.  For a pendulum, the dimensionless context is $\mathbf{d} = (q^*, \tau^*_{\max})
	= (q/(mgl),\ \tau_{\max}/(mgl))$; for a race car, the paper derives $m=6$ dimensionless
groups from the state and context variables (scaled positions and velocities).
% VERIFIED Girard2024: for the pendulum, "All three torque variables (tau, q, tau_max)
%   are scaled by the maximum gravitational torque" mgl — §II.2 Dimensional analysis,
%   p.6. For the car problem: "one control input, two states, and five context
%   parameters, for a total of 1 + (n=2) + (m=5) = 8 variables", yielding SIX Pi groups
%   (s*, x*, xdot*, x*_c, y*_c, q*) — §III-B, p.13. NOTE: the STEROID text previously
%   said "m=5 groups encoding mass ratios and scaled traction coefficients" — this is
%   not what the source states (the 6 groups are length/time scalings); corrected here.
The Buckingham Pi maps are power-law monomials and hence linear in log-space
...
% NOTE Girard2024 Theorem 1: states that "if a policy is physically meaningful...
%   the policy can be restated in a dimensionless form" — it is about the policy map
%   being expressible in dimensionless form, not specifically "power-law monomials".
%   The log-linearity claim follows from the Pi-group monomial construction rather
%   than Theorem 1 itself; reword to attribute this to the Buckingham Pi construction.
...
% VERIFIED Girard2024: exact quote "Equation (12) is a mapping from a m dimensional
%   space to a m - d dimensional space, and its inverse has multiple solutions" —
%   §III, p.8.
% VERIFIED Hromatko2025: "If two systems are dynamically similar (possessing the same
%   Pi-groups), their dimensionless dynamics are identical" and MPC parameters transfer
%   "direct transfer of closed-loop performance" — §II.B p.3.
...
% VERIFIED Gorard2025: flags "physically disallowed" combinations violating the
%   "Greenwald density limit", "plasma beta limit", and constraints causing "kink
%   instabilities" — Key Challenges, p.1. (The paper says "plasma beta limit", not
%   "Troyon beta limit" by name.)
% VERIFIED Sarazin2019: IPB98(y,2) and DS03 are power-law scaling laws for tau_E in
%   plasma temperature/density/field/geometry — Table 1 & eq. (20)/(29); the beta
%   limit beta_N,crit ~ 3 is discussed (ideal MHD) — §3.4. NOTE: cite Sarazin2019 for
%   the IPB98/DS03 scaling-law power-law claim (it is the dimensioning-study source).
...
nonlinear system solve per design point, not an LP.
% NOTE Sarazin2019: the source describes a "simple and comprehensive method" that
%   computes (R, B, beta_N) triplets satisfying target fusion gain and power given
%   the scaling law; it is a nonlinear constraint satisfaction per design point, so
%   the STEROID text's "self-consistent nonlinear system solve" is a fair but loose
%   paraphrase — consider softening to "nonlinear constraint satisfaction".
STEROID addresses the
...
When a certifiably exact solution is required, LP via HiGHS \cite{HiGHS2018}
...
% VERIFIED Bemporad2002: regions are polyhedral "critical regions" from the mp-QP
%   with active-set/KKT characterization — Algorithm 1, p.9.
...
noting the same regularity advantage.
% NOTE Scibilia2009: no PDF obtainable (ECC, closed). The attribution is to the
%   literature; the Delaunay-for-MPC idea is not independently verifiable from the
%   local library. Consider replacing with a PDF-backed reference on Delaunay-based
%   explicit MPC, or soften the attribution.
STEROID can be viewed as an exact variant
...
% NOTE Hormann2017: CRC book, no PDF in Zotero; generalized barycentric coordinates
%   are a standard meshfree interpolation tool (book is the canonical reference).
...
The current implementation assumes that the feasible set $\{\mathbf{p} : \Lambda\mathbf{p} \leq \mathbf{c}\}$
...
per component.
% FIX: previous text had "Lambda p >= c" (sign error); corrected to "<= c" to match
%   eq. (1) (Lambda p <= c) throughout the paper.
